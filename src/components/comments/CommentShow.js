...
import { removeCommentSuccess, removeCommentFailure } from '../shared/AutoDismissAlert/messages'
...
const CommentShow = (props) => {
    ...
    const deleteComment = () => {
        ...
        removeComment(user, transaction._id, comment._id)
            // send a success message
            .then(() =>
                msgAlert({
                    heading: `comment Deleted!`,
                    message: removeCommentSuccess,
                    variant: 'success',
                })
            )
        ...
        <>
            <Card key={idx} className='m-2'>
                <Card.Body>
                    <Row>
                        <Col md={8}>
                            <small>{idx}-{comment.content}</small>
                        </Col>
                        <Col md={4}>
                            {
                            user && transaction.owner && user._id === transaction.owner._id
                            ?
                            <>
                                <Button 
                                    variant="link"
